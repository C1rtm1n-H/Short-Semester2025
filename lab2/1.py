...
def main():
    ...
    df["age"] = df.groupby("class")["age"].transform(lambda x: pd.to_numeric(x, errors='coerce').fillna(x.median()))

    # 3) 打标签：passed，level
    df["passed"] = df["score"].apply(lambda s: s >= 60) 
    # level 的规则：>=90 优秀；80–89 良好；60–79 及格；其他 不及格
    # ——在这里用 Series.apply(lambda x: ...) 写出 level 列——
    df["level"] = df["score"].apply(lambda s: "优秀" if s >= 90 
                                    else "良好" if s >= 80 
                                    else "及格" if s >= 60 
                                    else "不及格")
    ...
    df['zscore'] = g.transform(lambda x: (x - x.mean()) / (x.std() if x.std() else 1)).round(3)
    ...
    summary = (df.groupby(['class', 'level'])
                 .agg(avg_age=('age', 'mean'), avg_score=('score', 'mean'))
                 .reset_index())
