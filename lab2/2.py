...
def deep_merge(*dicts, list_policy="extend"):
    ...
    def merge(a, b):
        if isinstance(a,dict) and isinstance(b,dict):
            merged = {key: merge(a[key], b[key]) for key in a if key in b}
            merged.update({key: a[key] for key in a if key not in b})
            merged.update({key: b[key] for key in b if key not in a})
            return merged
